{ config, pkgs, lib, ... }: with pkgs; with lib; let
  ...
  artifacts = "${artifactRoot}/lib/libclion_demo*.dll";
  release = "${artifactRoot}/lib/libclion_demo.dll";
in
{
  config = {
    name = "libclion_demo";
    ...
    artifactPackage = runCommand "example-artifacts" { } (''
      mkdir -p $out/lib
      cp ${config.artifactPackages.main}/lib/libclion_demo.dll $out/lib/
    '' + concatStringsSep "\n" (mapAttrsToList (key: addonPath: ''
        cp ${addonPath}/lib/libclion_demo.dll $out/lib/libclion_demo-${key}.dll
